...
def training_epoch(model: LanguageModel, optimizer: torch.optim.Optimizer, criterion: nn.Module,
    ...
    for indices, lengths in tqdm(loader, desc=tqdm_desc):
        ...
        # Пусть T = logits.shape[1] == lengths.max()
        T = logits.size(1)
        # Сдвигаем: для каждого временного шага t предсказываем token[t+1]
        logits = logits[:, :T-1, :]    # (batch_size, T-1, vocab_size)
        targets = indices[:, 1:T]        # (batch_size, T-1)
